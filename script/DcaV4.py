...
@app.callback(
    Output('initial-date-slider', 'max'),
    Output('initial-date-slider', 'marks'),
    Output('end-date-slider', 'max'),
    Output('end-date-slider', 'marks'),
    Input('well-name', 'value')
)
def update_sliders(well_name):
...
@app.callback(
    ...
    Input('well-name', 'value'), 
    Input('initial-date-slider', 'value'),
    Input('end-date-slider', 'value'),
    Input('months-end-date', 'value'),
    Input('oil-rate-intervention', 'value')
)
def update_plots(well_name, initial_slider_value, end_slider_value, months_end_date, rate_intervention):
    df = pd.read_excel(excel_file, sheet_name=well_name)
    df['DATE_STAMP'] = pd.to_datetime(df['DATE_STAMP'], format='%d/%m/%Y', dayfirst=True)
    
    start_date = df['DATE_STAMP'].iloc[initial_slider_value]
    end_date = df['DATE_STAMP'].iloc[end_slider_value]
    ...
    forecast_df = pd.DataFrame({'DATE': df_oil['DATE_STAMP'], 'TIME': range(len(df_oil['DATE_STAMP']))})
    ...
    oil_fig.add_trace(go.Scatter(x=[start_date], y=[df.loc[initial_slider_value, 'CORR_OIL_RATE_STBD']], mode='markers', name='Selected Start Date', marker=dict(color='black', size=8)))
    oil_fig.add_trace(go.Scatter(x=[end_date], y=[df.loc[end_slider_value, 'CORR_OIL_RATE_STBD']], mode='markers', name='Selected End Date', marker=dict(color='black', size=8)))
